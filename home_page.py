...
df_puppy_weights["ounces"] = (
    ((df_puppy_weights["pounds_float"] - df_puppy_weights["pounds"]) * 16)
    .round(2)
    .astype(float)
)
df_puppy_weights["weight-lboz"] = df_puppy_weights.apply(format_weight, axis=1)
df_puppy_weights.drop(["pounds_float", "pounds", "ounces"], axis=1, inplace=True)

# shape data and show a line chart
df_puppy_weights_pivoted = df_puppy_weights.pivot(index="date", columns="name")[
    "weight-gm"
]
...
st.line_chart(
    df_puppy_weights_pivoted,
    y=df_puppy_weights_pivoted.columns,
    x_label="2024",
...
if st.checkbox("Show all puppy weight data", value=True):
    ...
    puppies = st.multiselect("Names: ", names, names)
    st.dataframe(
        df_puppy_weights.loc[df_puppy_weights.name.isin(puppies)].sort_values(
